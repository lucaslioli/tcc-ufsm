...
\label{sec:exp-class}
    ...
    \begin{figure}[!ht]
    \caption{Análise da taxa de popularidade pelo uso de URLs e \textit{hashtags}. No eixo Y, N corresponde a Não e S corresponde a Sim}
    \centering
    \subfigure[ref1][Popularidade X uso de URLs]
    	{
            \begin{tikzpicture}[scale=0.8]
            \begin{axis}[
                xbar,
                ...
                legend style={at={(0.5,1.23)},
                  anchor=north,legend columns=2},
                ylabel={Uso de URL},
                symbolic y coords={N, S},
                ytick=data,
                nodes near coords,
                nodes near coords align={horizontal},
                legend image code/.code={%
                   \draw[#1, draw=none] (0cm,-0.2cm) rectangle (0.6cm,0.2cm);
                },
            ]
            \legend{Retuítes,Curtidas}
            \addplot [xbar, fill=blue!65,postaction={pattern=vertical lines}]
            	coordinates {(3053,S) (3759,N)};
            \addplot [xbar, fill=blue!15,postaction={pattern=north east lines}] 
            	coordinates {(12928,S) (9822,N)};
            
            \end{axis}
            \end{tikzpicture}
        }
    \hspace{0.1cm}
    \subfigure[ref1][Popularidade X uso de \textit{Hashtags}]
    	{
            \begin{tikzpicture}[scale=0.8]
            \begin{axis}[
                xbar,
            ...
            \legend{Retuítes,Curtidas}
            \addplot [xbar, fill=blue!65,postaction={pattern=vertical lines}]
            	coordinates {(1395,S) (3915,N)};
            \addplot [xbar, fill=blue!15,postaction={pattern=north east lines}] 
            	coordinates {(4483,S) (15247,N)};
