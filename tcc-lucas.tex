...
% % % DEDICATÓRIA
\dedicatoria{À família!}
% % % %  AGRADECIMENTOS
\agradecimentos{Aos professores e colegas que, sem eles, nada disso seria possível!}
...
\label{sec:arvores-decisao}
    ...
    \par Ainda conforme \cite{book:han:11}, para realizar a classificação de dados, cada registro percorre um determinado caminho dentro da estrutura da árvore, partindo do nó raiz até o nó folha, o qual determina a classe para aquela entrada de dados. Para exemplificar, foi criada a árvore de decisão fictícia apresentada na Figura \ref{fig:exemplo_arvore}, a qual considera 4 atributos de uma mensagem de texto para considerá-la como popular ou não. Na figura, os nós com o formato retangular representam os testes feitos cada registro de entrada, já os nós com o formato circular são os nós folha, que representam a classificação final para indicar se a mensagem seria considerada como popular ou não.
    ...
    \begin{tikzpicture}
      ...
      \node [env] {Sentimento positivo?}
        child { node [env] {Maior que 140 caracteres?}
            child { node [dummy] {não} edge from parent node [above] {sim}}
            child { node [env] {Possui \textit{hashtag}?}
                child { node [dummy] {não} edge from parent node [above] {sim}}
    ...
    \par Um dos algoritmos que merece destaque por ser referência neste âmbito, é o CART (\textit{Classification and Regression Trees}), criado em 1984 por um grupo de estatísticos (L. Breiman, J. Friedman, R. Olshen, and C. Stone), ele realiza uma abordagem de construção recursiva de divisão e conquista partindo de cima para baixo, segundo \cite{book:han:11}. Outros algoritmos serão apresentados e descritos conforme forem utilizados e aplicados na Seção \ref{sec:experimentos}, correspondente aos experimentos.
...
\label{sec:redes-neurais}
    ...
    \begin{figure}[ht]
        ...
        \vspace{\baselineskip} %%% linha em branco para atender a norma
        \fonte{\citeonline{site:deep-learning}}
        \label{fig:deep-learning}
    \end{figure}
    
    \par Existem vários modelos de arquiteturas de redes neurais propostos, dentre eles, vale a pena citar: Redes Multicamadas Perceptrons; Redes de Hopfield; Redes Neurais Convolucionais; e \textit{Long Short-Term Memory} (LSTM) (pode ser traduzido como Memória de Longo Prazo). No contexto deste trabalho, durante a seção \ref{sec:experimentos}, correspondente aos experimentos, será dado destaque à arquitetura de redes convolucionais e LSTM, por se destacarem e ser muito utilizada no processamento de linguagem natural.

    % \par No contexto deste trabalho, a aplicação de RNNs torna-se interessante aliada ao conceito de \textit{word-embeddings} ou \textit{one-hot vectors}. 
...
\label{sec:classificadores}
    ...
    \par Para a realização da predição da popularidade de cada tuíte tendo como base a taxa de engajamento e considerando os atributos já mencionados, serão utilizados os seguintes algoritmos de aprendizado de máquina: Naive Bayes; Árvores de decisão; e Redes Neurais Recorrentes, Convolucionais e LSTM. Os experimentos realizados com cada um destes algoritmos poderão ser realizados através da implementação de código na linguagem Python, devido a sua popularidade para lidar com aprendizado de máquina e manipulação de dados, ou com o auxilio da ferramenta Weka, que já oferece uma coleção de algoritmos capazes de realizar a classificação de dados.
    
    \par Os algoritmos de classificação foram escolhidos com base em sua popularidade e destaque para solucionar o problema de classificação. Todos eles inclusive, são frequentemente analisados por pesquisado em artigos que utilizam o aprendizado de máquina como foco.
...
\label{sec:metodologia}
    ...
    \par Para alcançar os objetivos elencados, uma série de etapas foi elaborada para a realização deste projeto. A fim de proporcionar maior compreensão, a descrição de cada etapa vem logo a seguir.
    
    \par \textbf{Análise de trabalhos similares:} Nesta etapa será realizada pesquisa sobre trabalhos similares que podem vir a auxiliar e contribuir com a proposta do trabalho.
    ...
    \par \textbf{Extração e pré-processamento dos dados:} Dispondo dos atributos de interesse, os tuítes devem ser coletados e seus conteúdos pré-processados. Para isso, será realizado um estudo acerca dos melhores métodos e ferramentas que permitem a extração de dados do Twitter.
    ...
    \par \textbf{Experimentos com os algoritmos selecionados:} Esta etapa corresponde a implementação e realização de testes com os algoritmos estudados na etapa anterior. O experimentos serão realizados com o objetivo de classificar a popularidade dos tuítes com base nos atributos extraídos.
    
    \par \textbf{Análise dos resultados obtidos:} Esta etapa do trabalho destina-se a realização de uma análise sobre os algoritmos testados e os resultados obtidos para a realizar a predição da popularidade dos tuítes.
    
    A Tabela \ref{tab:cronograma} a seguir resume o cronograma de atividades baseado no itens apresentados. Dividido entre meses, o cronograma tem início a partir do mês de Agosto. As atividades já realizadas até o momento compreendem os itens marcados com bola ($\bullet$) e fundo em cinza, enquanto que os marcadores em xis ($\times$) representam as atividades que ainda devem ser realizadas.
    ...
    Análise de trabalhos similares & \cellcolor[HTML]{E2E2E2}$\bullet$ &  &  &  &  \\ \hline
    Definição dos Atributos & \cellcolor[HTML]{E2E2E2}$\bullet$ & \cellcolor[HTML]{E2E2E2}$\bullet$ &  &  &  \\ \hline
    Extração e pré-processamento dos dados &  & \cellcolor[HTML]{E2E2E2}$\bullet$ & \cellcolor[HTML]{E2E2E2}$\bullet$ &  &  \\ \hline
    Estudo de algoritmos de aprendizado de máquina &  &  & \cellcolor[HTML]{E2E2E2}$\bullet$ & $\times$ &  \\ \hline
    Experimentos com os algoritmos selecionados &  &  & \cellcolor[HTML]{E2E2E2}$\bullet$ & $\times$ & $\times$ \\ \hline
    Análise dos resultados obtidos &  &  &  & $\times$ & $\times$ \\ \hline
    \end{tabular}
    \end{table}
