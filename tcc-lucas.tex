...
% % % DATA DA DEFESA 
\data{29}{11}{2018} %% FORMATO {DD}{MM}{AAAA}
...
\introducao{
    ...
    \par Dentre os trabalhos citados logo acima, tal e tal utilizam as técnicas tais para fazer tal coisa. Enquanto que os trabalhos tal, tal e tal propõe a realização de tal coisa utilizando as técnicas tais de aprendizado de máquina.
    
    % ? Definir ou agrupar por características, cada um dos trabalhos. Apresentar que o que pretende-se nesse trabalho, diferentemente dos demais, é que utilizamos contas influentes e tentamos predizer uma taxa de engajamento para cada conta. Definir um cenário para cada conta que o alcance pode ser previsto até uma determinada taxa. ?

    \par Dentro deste contexto, o objetivo deste trabalho é monitorar e extrair tuítes de determinadas contas do Twitter, a fim de elaborar modelos, utilizando algoritmos de aprendizado de máquina, para realizar a predição e classificação da popularidade de tuítes com base em suas características. Devido ao grande volume de dados, faz-se necessário automatizar o processo de análise e classificação dos dados, para isso, serão estudados e testados algoritmos já consolidados, como Naive Bayes e árvores de decisão, contando inclusive com o auxílio de ferramentas já reconhecidas como Weka, para a aplicação e análise dos resultados destes algoritmos. Como entrada para estes algoritmos, serão utilizados dados provenientes do pré-processamento dos tuítes coletados, sendo consideradas três características que não foram contempladas pelo estudo de \cite{ieee:suh:10}: o tamanho em caracteres, o sentimento (que mede a emoção transmitida) e a banalidade (que mede a relevância da mensagem). Para fins de comparação, a presença de \textit{hashtags} e URLs também foi avaliada.
...
\label{sec:redes-neurais}
    ...
    \par Existem vários modelos de arquiteturas de redes neurais propostos, dentre eles, vale a pena citar: Redes Multicamadas Perceptrons; Redes de Hopfield; Redes Neurais Convolucionais; e \textit{Long Short-Term Memory} (LSTM) (pode ser traduzido como Memória de Longo Prazo). 
