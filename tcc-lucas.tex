...
\introducao{
    ...
    %\par É conhecido que hoje existem inúmeras pesquisas sendo realizadas envolvendo dados coletados do Twitter. Alguns trabalhos relacionados que devem ser citados aqui, como tal, tal e tal, que tem como parte de seus objetivos, a identificação de fatores impactantes no conteúdo das mensagens. Em tal, o estudo envolveu tais tópicos. No trabalho de tal, o foco foi tal tecnologia...
    
    % Separar por características dos trabalhos. O que pretende-se nesse trabalho, diferentemente dos demais, é que utilizamos contas influentes e tentamos predizer uma taxa de engajamento para cada conta. Definir um cenário para cada conta que o alcance pode ser previsto até uma determinada taxa.
...
\label{sec:redes-neurais}
    ...
    \par As funções de ativação e maneira como a ligação entre os neurônios é feita pode variar, porém, existem duas formas para a realização destas conexões que configuram tipos de redes distintas: redes neurais com alimentação para frente (ou \textit{feed-forward}), em que suas conexões são realizadas em uma única direção, sem a presença de laços; e as Redes Neurais Recorrentes, comumente chamadas pela sigla RNN (do inglês \textit{Recurrent Neural Network}), as quais possuem laços que alimentam as entradas com suas próprias saídas, o que torna sua compreensão complexa, já que ``a resposta da rede para determinada entrada depende do seu estado inicial, que pode depender de entradas anteriores'' \cite{book:russell:10}.
...
\label{sec:metodologia}
    ...
    % Descrever as etapas e colocar o cronograma
