...
\label{sec:exp-engaj-balanc}
    ...
    \par A primeira coisa que pode ser observada, é que para cada uma das curvas, o pico é determinado por taxas de engajamento diferentes. O que reforça o propósito da aplicação dos modelos considerando contas individualizadas, já que a taxa de engajamento ideal para um usuário, pode não ser a mesma para os demais. No caso de analisar apenas a curva ''Geral``, é possível observar que o valor ótimo para maximizar o número de dados balanceados é de 2\%, sendo que a variação desta taxa, para qualquer direção, resulta em uma perda de dados considerável, padrão que também se aplica aos demais casos.
...
\label{sec:exp-individual}
    ...
    % DADOS
    \pgfplotstableread[row sep=\\,col sep=&]{
        algorithm       & uTrump & txEllen & txJones \\
        NBayesTexto     & 0.557  & 0.617   & 0.618   \\
        NBayesAtributos & 0.615  & 0.596   & 0.655   \\
        J48             & 0.625  & 0.558   & 0.662   \\
        LMT             & 0.611  & 0.561   & 0.662   \\
        }\mydata
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            \addplot [fill=myblue!75,postaction={pattern=north east lines, pattern color=Blue}] table[x=algorithm,y=uTrump]{\mydata};
            
            \addplot [fill=myred!70,postaction={pattern=horizontal lines, pattern color=Mahogany}] table[x=algorithm,y=txEllen]{\mydata};
            
            \addplot [fill=YellowOrange,postaction={pattern=vertical lines, pattern color=Yellow}] table[x=algorithm,y=txJones]{\mydata};
            
            \legend{Donald J. Trump, Ellen DeGeneres, Alex Jones}
        \end{axis}
    \end{tikzpicture}
    \fonte{Produção do próprio autor.}
    ...
    \pgfplotstableread[row sep=\\,col sep=&]{
        algorithm       & uTrump & txEllen & txJones \\
        NBayesTexto     & 0.814  & 0.499   & 0.744   \\
        NBayesAtributos & 0.795  & 0.494   & 0.681   \\
        J48             & 0.743  & 0.430   & 0.664   \\
        LMT             & 0.765  & 0.544   & 0.664   \\
        }\mydata
...
\label{sec:conclusao}
    ...
    %\par Apesar de incipientes, os resultados alcançados mostram que as características avaliadas parecem exercer influência no nível de interesse, e podem ser levadas em consideração na elaboração de estratégias que visem impulsionar publicações. 
